...
public class Ray : Curve
{
    ...
    /// 创建射线的新实例
    /// </summary>
    /// <param name="origin">线端点</param>
    /// <param name="direction">方向</param>
    /// <returns>新的有界直线</returns>
    /// <exception cref="ArgumentNullException">当origin或者direction为null时抛出</exception>
    /// <exception cref="ArgumentOutOfRangeException">direction不可为0长向量</exception>
    public static Ray Create(XYZ origin, XYZ direction)
    {
        if (origin == null)
        {
    ...
    public override double Length
    {
        get
        {
            throw new ArgumentException("无边界的线无法计算长度");
        }
    }
    ...
    {
        return Create(StartPoint, Direction);
    }
    ...
    {
        return Create(StartPoint, Direction.Negate());
    }
    ...
    public override Curve CreateOffset(double offsetDist, XYZ referenceVector)
    {
        ...
        {
            throw new Exception("参考向量与线方向平行，无法确定平移方向，不能创建平移！");
        }
        offsetVector = offsetVector.Normalize();

        XYZ origin_Offset = Origin.Add(offsetVector * offsetDist);
        return Create(origin_Offset, Direction);
    }
    ...
    public override XYZ GetEndPoint(int index)
    {
        ...
        {
            throw new ArgumentOutOfRangeException("射线为无界的");
        }
    }
