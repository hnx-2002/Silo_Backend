...
public class Polyline : Curve
{
    ...
    /// <exception cref="ArgumentNullException">顶点数量小于2/两顶点形成的多段线不能闭合</exception>
    /// <exception cref="ArgumentException">两点间距过小</exception>
    public static Polyline Create(List<XYZ> points, bool isClosed = false)
    {
        if (points.Count < 2)
        {
        ...
        if (points.Count == 2 && points.First().IsAlmostEqualTo(points.Last()))
        {
        ...
        polyline.StartPoint = points.First();
        polyline.EndPoint = points.Last();
        return polyline;
    }
