...
import { NButton, NInputNumber, NFlex, NIcon } from 'naive-ui';
...
function preview(filePath: string, fileName: string, width: number, height: number) {
...
function downloadFile() {
  //TODO 根据currentFilePath.value获取文件名
  frontDownload(currentFilePath.value, '');
  // currentFilePath.value
}

//改变呈现页码
function changeCurrentPage(data: number) {
  pageNum.value = data;
}

//修改比例
function changePageScale(val: number) {
  Previewer_PDFRef.value.updateCurrentPageScale(val);
}

//旋转页面
function rotatePage(val: boolean) {
