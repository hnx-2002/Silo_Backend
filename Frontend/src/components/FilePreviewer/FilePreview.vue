...
import Previewer_PDFWithToolBar from '@common/FilePreviewer/PDFJS/Previewer_PDFWithToolBar.vue';
import Previewer_M3U8 from '@common/FilePreviewer/M3U8/Previewer_M3U8.vue';
...
//预览
function preview(filePreviewPath) {
    ...
    setTimeout(() => {
        if (fileType.value == 'mp4') {
            ...
            if (fileType.value == 'pdf') {
                Previewer_PDFWithToolBarRef.value.preview(filePath.value, '', '', '660px');
                ...
                const myXlsxPreviewer = JsPreviewExcel.init(
                    document.getElementById('xlsx-container')
                );
