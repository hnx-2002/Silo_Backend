<script setup>
    ...
    let pages = {}; // 用于存储每一页的 page 对象
    let canvases = {}; // 用于存储每一页的 canvas 对象

    const loadPercent = ref(0); // 加载百分比
    const pageStates = ref({}); // 用于存储每个页面的状态，包括缩放和旋转
    ...
    function previewPDF(inPath, fileTitle, domW, domH) {
        loadPercent.value = 0;
                    ...
                    for (let i = 1; i <= numPages; i++) {
                        renderPromise = renderPromise.then(() => {
                            return pdf.getPage(i).then((page) => {
                                ...
                                loadPercent.value = parseFloat(
                                    (parseFloat(tempPercent) * 100).toFixed(2)
                                );
    ...
    function renderPage(page, parentDom, pageIdx) {
    ...
    function createCanvas(page, pageIdx) {
    ...
    function createRenderContext(page, canvas, pageIdx) {
    ...
    function getCurrentScrollNum() {
        return pdfContainer.scrollTop;
    }

    function setCurrentScrollNum(val) {
        pdfContainer.scrollTop = val;
    }

    function pageTo(pageIdx) {
    ...
    function updateCurrentPageScale(changeScale) {
        ...
        if (oldCanvas && page) {
            ...
            const viewport = page.getViewport({ scale, rotation });
            const renderContext = {
                canvasContext: oldCanvas.getContext('2d'),
                viewport: viewport,
            };
    ...
    function rotateCurrentPage(isClockwise) {
        const pageIdx = currentPageNum.value;
        const oldCanvas = canvases[pageIdx]; // 使用存储的 canvas
        const page = pages[pageIdx]; // 使用存储的 page
        ...
        if (oldCanvas && page) {
            ...
            if (isClockwise) {
                ...
                pageStates.value[pageIdx].rotation =
                    (pageStates.value[pageIdx].rotation - 90 + 360) % 360;
            }
...
<template>
    <NFlex vertical>
        <NProgress v-if="loadPercent < 100"
                   type="line"
